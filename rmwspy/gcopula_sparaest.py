...
def paraest_multiple_tries(
                ...
                talk_to_me=True,
                use_timeseries_fitting=True, #! flag to use modified fitting approach based on sampling all gauges at n_subsets timesteps
                n_subsets=None, #! number of subsets (timesteps) to use in fitting
                ):
        ...
        for subset in range(ntries[1]):
            ...
            for startings in range(ntries[0]):
                ...
                out000 = paraest_g(
                        ...
                        outputfile=None,
                        talk_to_me=True,
                        use_timeseries_fitting=use_timeseries_fitting, #! flag to use modified fitting approach based on sampling all gauges at n_subsets timesteps
...
def paraest_g(
    ...
    # choose between original and modified (timeseries-based) fitting approaches
    if use_timeseries_fitting:
        # modified method - randomly generate required number of indices of timesteps to use in fitting
        n_timesteps = u.shape[1]
        ind = build_subsets2(n_timesteps, n_subsets)
        
        # repeat locations matrix for each selected timestep, just so that likelihood
        # function does not need to be changed
        x0 = np.zeros((n_subsets, x.shape[0], x.shape[1]))
        x0[:] = x[:]
        
        # subset rank-transformed values based on randomly generated indices
        # - swap axis order again so that likelihood function can stay the same
        u0 = u[:,ind]
        u0 = np.swapaxes(u0, 1, 0)
    else:
        # original method
        ind = build_subsets(
                            x,
    ...
    Rangebounds = [[1.0, d0.max() * 2]]  # more flexible range bounds for MC
    p_bounds += Rangebounds

    # Sill bounds added for MC (avoid minimum of zero)
    p_bounds.append([0.1, 1])
...
def reconstruct_parameters(p, covmods):
    ...
    Range = p[counter]
    counter += 1
    # cov_model += '1.0 %s(%1.12f)'%(covmods, Range) # original line
    cov_model += ' %s(%1.12f)'%(covmods, Range) # new cov_model with space for calculated sill instead of 1.0

    sill = p[counter] # calculate sill at every step in the loop
    cov_model = '%1.12f'%(sill) + cov_model # include final calculated sill in cov_model string

    counter += 1 # sill counter
...
# alternative way to build subsets in the case that the number of gauges is 
# small but the number of timesteps is relatively large (i.e. repeated
# sampling of a set of gauges)
def build_subsets2(n_timesteps, n_subsets=None):
    
    #! check that n_subsets does not exceed n_timesteps
    if (n_subsets is None) or (n_subsets > n_timesteps):
        n_subsets = n_timesteps
    
    # randomly pick some timestep indices (which will help us put all gauges for
    # the relevant timesteps into the fitting procedure)
    ind = np.random.randint(0, n_timesteps, n_subsets)
