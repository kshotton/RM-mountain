...
class RMWS(object):
    def __init__(self,
                 #! nonlinearproblem,
                 nonlinearproblem=None,     # set default to None so that a NonLinearProblemTemplate is not needed for a linear problem
        ...
        # only check NonLinearProblemTemplate if needed
        if nonlinearproblem is not None:
        ...
        if self.method == 'no_nl_constraints':
            # TESTING self.n_uncondFields = [np.min((np.max((self.cp.shape[0] + self.le_cp.shape[0] + self.ge_cp.shape[0], 500)), 5000))]

            # TESTING
            n_points = self.cp.shape[0] + self.le_cp.shape[0] + self.ge_cp.shape[0]
            self.n_uncondFields = [np.min((np.max((n_points, 50)), 5000))]
    ...
    def find_low_norm_weights(self,):
        ...
        while norm_inner > 0.1:
            ...
            if n > self.n_uncondFields[0]:                
                # TESTING ix,jx = self.add_uncondFields(nF=[1000])
                ix, jx = self.add_uncondFields(nF=[10])
    ...
    def index_gen(self, inds, n):
        ...
        while True:
            ...
            if len(res) < n:
                # TESTING ix, jx = self.add_uncondFields(nF=[2000])
                ix, jx = self.add_uncondFields(nF=[10])  # TESTING
                indit = iter(jx)
