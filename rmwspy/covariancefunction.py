...
def type_exp(h, Range=1.0, Sill=1.0):
    ...
    return Sill * (np.exp(-h / Range)) + (1.0 - Sill)  # including offset for Marmot Creek Research Basin

# Spherical
def type_sph(h, Range=1.0, Sill=1.0):
    h = np.array(h)
    return np.where(h>Range, 0, Sill * (1 - 1.5*h/Range + h**3/(2*Range**3)))
...
def find_maximum_range(model='1.0 Exp(1.0)', rho_thresh=0.03):
    ...
    # TESTING - try looking instead for distance beyond which change in correlation is very small
    integralscale = 0.0
    prev_corr = Nested_Cov(integralscale, model=model)
    corr_diff = 999
    while corr_diff > 0.000001:
        integralscale += maxrange / 10.0
        corr = Nested_Cov(integralscale, model=model)
        corr_diff = prev_corr - corr
        prev_corr = corr
    integralscale = max(maxrange*3,   integralscale)
    integralscale = min(maxrange*100, integralscale)
