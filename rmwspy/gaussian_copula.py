...
def multivariate_normal_copula_pdf(u, Cov):
    ...
    # vorfaktor = 1.0 # test some fixed values for vorfaktor
    # vorfaktor = 10.0
    exponent = -0.5 * (((np.tensordot(x, np.linalg.inv(Cov), axes=1)) * x).sum(axis=1))

    # try: # this block stops the code and prints Cov if there is a divide by zero problem in calculating exponent
    #     exponent = -0.5 * (((np.tensordot(x, np.linalg.inv(Cov), axes=1)) * x).sum(axis=1))
    # except:
    #     print(Cov)
    #     sys.exit("stop")
